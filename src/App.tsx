...
const generateFluxFeature = (prevResult: string | null, forceMatch: boolean): { stim: StimulusData, result: string } => {
  ...
  const codeColor = generateCode([]);
  const codeShape = generateCode([codeColor]);
  const codeExact = generateCode([codeColor, codeShape]);
  const dict = shuffleEntries({ [codeColor]: 'MATCH_COLOR', [codeShape]: 'MATCH_SHAPE', [codeExact]: 'EXACT_MATCH' });

  const start = { shape: getRandomItem(SHAPES), color: getRandomItem(COLORS) };
  let end = { ...start };
  ...
  let activeCode = '';
  if (result === 'MATCH_COLOR') activeCode = codeColor;
  else if (result === 'MATCH_SHAPE') activeCode = codeShape;
  else if (result === 'EXACT') activeCode = codeExact;
  else activeCode = getRandomItem([codeColor, codeShape, codeExact]);
  ...
  return {
    stim: {
      ...
      visuals: { start, end },
      textQuery: `VERIFY: ${activeCode}`,
      logicProof: `(Color: ${start.color === end.color ? '=' : '!='}) & (Shape: ${start.shape === end.shape ? '=' : '!='})`
    },
    result
  };
};

// 2. FLUX COMPARISON (Swapped Visuals Update)
const generateFluxComparison = (prevResult: string | null, forceMatch: boolean): { stim: StimulusData, result: string } => {
  ...
  // Randomly decide which Leaf is visually on the left.
  // This decouples "Left Visual" from "Left Operand in Query"
  const visualSwap = Math.random() > 0.5;
  const visualLeft = visualSwap ? leaf2 : leaf1;
  const visualRight = visualSwap ? leaf1 : leaf2;
  ...
  const c1 = getRandomItem(colorOptions);
  const c2 = getRandomItem(colorOptions.filter(c => c.name !== c1.name));

  const iconBase = getRandomItem(ICONS);
  const meaning1 = Math.random() > 0.5 ? '>' : '<';
  const meaning2 = meaning1 === '>' ? '<' : '>';

  const dict = shuffleEntries({
    [`${iconBase} (${c1.name})`]: meaning1,
    [`${iconBase} (${c2.name})`]: meaning2
  });

  // Calculate logic for Leaf1 vs Leaf2 (The Query)
  let targetRel1 = result === 'GREATER' ? '>' : '<'; // Leaf1 relation to Hub
  let targetRel2 = result === 'GREATER' ? '<' : '>'; // Leaf2 relation to Hub

  // Determine needed colors for Leaf1 and Leaf2 logic
  const colorForLeaf1 = meaning1 === targetRel1 ? c1 : c2;
  const colorForLeaf2 = meaning2 === targetRel2 ? c2 : c1;

  // Assign colors to VISUAL positions (Left/Right)
  // If visualLeft is Leaf1, it gets colorForLeaf1.
  const visualLeftColor = visualSwap ? colorForLeaf2 : colorForLeaf1;
  const visualRightColor = visualSwap ? colorForLeaf1 : colorForLeaf2;
  ...
  return {
    stim: {
      ...
      dictionary: dict,
      dictionaryPos: Math.random() > 0.5 ? 'LEFT' : 'RIGHT',
      contextColors: [visualLeftColor.class, visualRightColor.class],
      visuals: {
        hub,
        leftLeaf: visualLeft, // This might be Leaf2!
        rightLeaf: visualRight, // This might be Leaf1!
        icon: iconBase,
        isSwapped: visualSwap
      },
      textQuery: `DERIVE: ${leaf1} vs ${leaf2}`, // Query remains constant
...
const generateFluxSpatial = (prevResult: string | null, forceMatch: boolean): { stim: StimulusData, result: string } => {
  const relations = ['NORTH_EAST', 'NORTH_WEST', 'SOUTH_EAST', 'SOUTH_WEST'];
  let result = getRandomItem(relations);
  if (forceMatch && prevResult && relations.includes(prevResult)) result = prevResult;
  else if (!forceMatch && prevResult) result = getRandomItem(relations.filter(r => r !== prevResult));

  const codeN = generateCode([]);
  const codeS = generateCode([codeN]);
  const codeE = generateCode([codeN, codeS]);
  const codeW = generateCode([codeN, codeS, codeE]);

  const dict = shuffleEntries({ [codeN]: 'NORTH', [codeS]: 'SOUTH', [codeE]: 'EAST', [codeW]: 'WEST' });

  let moves: string[] = [];
  if (result === 'NORTH_EAST') moves = [codeN, codeE];
  if (result === 'NORTH_WEST') moves = [codeN, codeW];
  if (result === 'SOUTH_EAST') moves = [codeS, codeE];
  if (result === 'SOUTH_WEST') moves = [codeS, codeW];
  ...
  return {
    stim: {
      ...
      textQuery: 'NET VECTOR FROM START',
      logicProof: `Sum(${moves.join(', ')}) = ${result}`
    },
    result
  };
};
...
const generateFluxDeictic = (prevResult: string | null, forceMatch: boolean): { stim: StimulusData, result: string } => {
  ...
  // Limit faces to prevent overwhelming rotation confusion initially
  ...
  // Logic: 
  // Identity: ME = 0, YOU = 2 (+180deg)
  // Time: NOW = 0, THEN = 1 (+90deg clockwise)

  const idShift = activeCode === codeMe ? 0 : 2;
  const timeShift = timeFrame === 'NOW' ? 0 : 1;
...
const generateFluxConditional = (prevResult: string | null, forceMatch: boolean): { stim: StimulusData, result: string } => {
  ...
  const startColor = Math.random() > 0.5 ? 'RED' : 'BLUE';
  let needsInvert = startColor !== result;

  const codeKeep = generateCode([]);
  const codeInvert = generateCode([codeKeep]);

  const modifierCode = needsInvert ? codeInvert : codeKeep;

  const dict = shuffleEntries({
    [codeKeep]: 'KEEP_COLOR',
    [codeInvert]: 'INVERT_COLOR'
  });
  ...
  return {
    stim: {
      ...
      dictionary: dict,
      dictionaryPos: Math.random() > 0.5 ? 'LEFT' : 'RIGHT',
      visuals: { startColor, modifier: modifierCode },
      textQuery: 'DETERMINE FINAL COLOR',
      logicProof: `${startColor} + ${modifierCode} = ${result}`
    },
    result
  };
};

// 9. FLUX ANALOGY (Polysemy Update)
const generateFluxAnalogy = (prevResult: string | null, forceMatch: boolean): { stim: StimulusData, result: string } => {
  ...
  // Map 2 codes to CAUSES, 2 codes to PREVENTS (Polysemy)
  const dict = shuffleEntries({
    [c1]: 'CAUSES',
    [c2]: 'CAUSES',
    [c3]: 'PREVENTS',
  ...
  // Pick distinct symbols for the visual
...
const VisualRenderer: React.FC<{ stim: StimulusData }> = ({ stim }) => {

  // NEW: State to handle the "Ghost View" blurring
  const [revealHint, setRevealHint] = useState(false);

  // NEW: Reset the hint to hidden (blurred) whenever the turn changes
  useEffect(() => {
    setRevealHint(false);
  }, [stim]);
    ...
    return (
      <div className="flex flex-col gap-4 w-full items-center justify-center">
        <div className="relative flex items-center gap-2 md:gap-4 bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden px-6 py-6 md:px-12 md:py-8 w-full max-w-lg">
          ...
          <div className="relative z-10 w-10 h-10 md:w-12 md:h-12 bg-black/80 border border-slate-500 rounded-lg flex items-center justify-center text-yellow-400 text-xl md:text-2xl shadow-xl">{icon}</div>
          <div className="relative z-10 w-16 h-16 md:w-24 md:h-24 bg-white rounded-full border-4 border-slate-300 flex items-center justify-center text-3xl md:text-5xl font-black text-black shadow-2xl mx-2">{hub}</div>
          <div className="relative z-10 w-10 h-10 md:w-12 md:h-12 bg-black/80 border border-slate-500 rounded-lg flex items-center justify-center text-yellow-400 text-xl md:text-2xl shadow-xl">{icon}</div>
  ...
  // 3. OPPOSITION (Fixed: Uses dynamic labels from chain instead of hardcoded A/B/C)
  if (stim.type === 'FLUX_OPPOSITION') {
    const { chain } = stim.visuals;
    // chain = [{ l: 'X', icon: '...', r: 'Y' }, { l: 'Y', icon: '...', r: 'Z' }]
    return (
      <div className="flex flex-col gap-4 w-full items-center scale-75 md:scale-100">
        <div className="flex items-center gap-0">
          {/* Node 1 (Start) */}
          <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-slate-700 border-2 border-slate-500 flex items-center justify-center font-bold text-white z-10 text-xl">
            {chain[0].l}
          </div>

          {/* Link 1 */}
          <div className="w-24 h-8 md:w-32 md:h-10 bg-slate-800 border-y-2 border-slate-700 flex items-center justify-center -mx-2">
            <span className="text-purple-400 font-bold text-2xl">{chain[0].icon}</span>
          </div>

          {/* Node 2 (Middle) */}
          <div className="w-12 h-12 md:w-16 md:h-16 rounded-full bg-slate-700 border-2 border-slate-500 flex items-center justify-center font-bold text-slate-500 z-10 text-xl">
            {chain[0].r}
          </div>

          {/* Link 2 */}
          <div className="w-24 h-8 md:w-32 md:h-10 bg-slate-800 border-y-2 border-slate-700 flex items-center justify-center -mx-2">
            <span className="text-purple-400 font-bold text-2xl">{chain[1].icon}</span>
          </div>
  ...
  // 5. CAUSAL (Dynamic Flow)
  if (stim.type === 'FLUX_CAUSAL') {
    const { nodes, ops, isReverse } = stim.visuals;
    // nodes = [Start, Middle, End]
    // ops = [Op1, Op2]

    // If Reverse, we render: End <- Op2 <- Middle <- Op1 <- Start
    // If Normal, we render: Start -> Op1 -> Middle -> Op2 -> End
    ...
    return (
      <div className="flex flex-row items-center gap-2 md:gap-4 scale-75 md:scale-100">
        {/* Node 1 */}
        <div className="px-4 py-3 bg-slate-800 rounded-lg text-white font-bold border border-slate-600 shadow-lg">{renderNodes[0]}</div>

        {/* Link 1 */}
        <ArrowRight className={`text-slate-500 w-6 h-6 ${arrowRotation}`} />
        <div className="w-12 h-12 border-2 border-slate-600 bg-black rounded flex items-center justify-center text-purple-400 font-bold text-xl shadow-[0_0_10px_rgba(168,85,247,0.2)]">
          {renderOps[0]}
        </div>
        <ArrowRight className={`text-slate-500 w-6 h-6 ${arrowRotation}`} />

        {/* Node 2 */}
        <div className="px-3 py-2 bg-slate-900 text-slate-400 text-sm rounded border border-slate-800 font-mono">{renderNodes[1]}</div>
        ...
        {/* Node 3 */}
        <div className="px-4 py-3 bg-slate-800 rounded-lg text-white font-bold border border-slate-600 shadow-lg">{renderNodes[2]}</div>
      </div>
    )
  }
  ...
  // 7. DEICTIC (With Locked Blur Helper)
  if (stim.type === 'FLUX_DEICTIC') {
    ...
    const currentViewDir = dirNames[effectiveFaceIdx];

    return (
      <div className="flex flex-col items-center gap-4">
        <div className="relative">
          <div className="grid grid-cols-3 gap-1 md:gap-3 p-2 bg-slate-800 rounded-2xl border border-slate-700 shadow-inner">
            {cells.map((_, i) => (
              <div key={i} className="w-8 h-8 md:w-20 md:h-20 bg-slate-900 rounded-lg md:rounded-xl flex items-center justify-center relative transition-all">
                {i === targetPos && <div className="w-2 h-2 md:w-6 md:h-6 bg-yellow-400 rounded-full animate-pulse shadow-lg shadow-yellow-500/50" />}
                {i === activePos && (
                  <div className={`transform transition-all ${rot}`}>
                    <div className="w-0 h-0 border-l-[8px] border-r-[8px] border-b-[16px] md:border-l-[16px] md:border-r-[16px] md:border-b-[32px] border-l-transparent border-r-transparent border-b-emerald-500 filter drop-shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                    <div className="absolute -top-4 md:-top-8 left-1/2 -translate-x-1/2 text-[8px] md:text-xs font-bold text-purple-300 bg-black/80 px-1 rounded">{activeCode}</div>
                  </div>
                )}
              </div>
            ))}
          ...
          {/* --- UPDATE: Blurred Helper Button --- */}
          <button
            onClick={() => setRevealHint(true)}
            className="absolute -right-16 top-1/2 -translate-y-1/2 bg-slate-900 border border-slate-600 p-2 rounded flex flex-col items-center cursor-pointer hover:border-slate-400 transition-colors group"
            title="Click to Reveal Answer"
          >
            <div className="text-[10px] text-slate-500 uppercase group-hover:text-slate-300 transition-colors">View</div>
            <div className={`text-xl font-black text-white transition-all duration-300 ${revealHint ? 'blur-none' : 'blur-md select-none'}`}>
              {currentViewDir}
            </div>
            {!revealHint && <HelpCircle className="w-3 h-3 text-slate-600 mt-1 absolute bottom-1" />}
          </button>
          {/* ----------------------------------- */}

        </div>
        <div className="flex items-center gap-4 text-xs font-mono bg-slate-900/50 p-2 rounded">
          <div className="flex items-center gap-1">
            <Clock className="w-3 h-3 text-slate-400" />
            <span className={timeFrame === 'THEN' ? 'text-red-400' : 'text-emerald-400'}>{timeFrame}</span>
          </div>
          <span className="text-slate-600">|</span>
          <div className="text-slate-400">Target is to my...?</div>
        </div>
      </div>
    );
  }
...
// --- Helper Component for Feedback Blurring ---
const BlurredLogicBox = ({
  label,
  result,
  proof,
  isCurrent
  ...
  // Auto-hide if the data changes (new turn)
  useEffect(() => { setRevealed(false); }, [result, proof]);

  return (
    <button
      onClick={() => setRevealed(true)}
      className={`mt-auto w-full text-center relative group transition-all duration-200 text-left
        ${isCurrent
          ? 'p-3 bg-black rounded-xl border border-slate-700 shadow-lg'
          : 'p-2 bg-slate-950 rounded-xl border border-slate-800'
        }
      `}
    >
      <div className="text-[10px] text-slate-600 uppercase font-bold tracking-wider mb-1 flex justify-between items-center px-1">
        <span>{label}</span>
        {!revealed && <EyeOff className="w-3 h-3 opacity-50" />}
        {revealed && <Eye className="w-3 h-3 opacity-50 text-emerald-500" />}
      </div>

      <div className={`transition-all duration-300 ${revealed ? 'blur-none' : 'blur-md select-none opacity-50'}`}>
        <div className={`font-black ${isCurrent ? 'text-2xl text-white' : 'text-lg text-slate-300'}`}>
          {result}
        </div>
        <div className={`text-[10px] mt-1 font-mono p-1 rounded border
            ${isCurrent
            ? 'text-purple-400 bg-slate-900 border-slate-800'
            : 'text-slate-600 bg-black/50 border-transparent'
          }
          `}>
          {proof}
        </div>
      ...
      {/* Overlay Label to indicate clickability when hidden */}
      {!revealed && (
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="bg-slate-900/90 border border-slate-700 px-3 py-1 rounded-full text-[10px] font-bold text-slate-400 uppercase shadow-xl group-hover:text-white group-hover:border-slate-500 transition-colors">
            Click to Reveal
          </div>
        </div>
      )}
...
export default function ProjectOmegaUltimate() {
  ...
  const nextTurn = useCallback(() => {
    ...
    if (histLen < n && phase !== 'WARMUP') setPhase('WARMUP');
    if (histLen >= n && phase !== 'PLAYING') setPhase('PLAYING');

    setIsButtonsFlipped(Math.random() > 0.5);

    let type: GeneratorType = 'FLUX_FEATURE';
    if (config.isPracticeMode && config.practiceType) {
    ...
    const prevNItem = histLen >= n ? history[histLen - n] : null;
    ...
    if (config.baseTimer === -1) { setTimer(100); }
      ...
      setTimer(Math.max(3, config.baseTimer - (difficultyMod * 4)));
    }
    startTimeRef.current = Date.now();
  }, [activeElo, history, config, phase]);
  ...
  const handleAnswer = useCallback((userMatch: boolean) => {
    ...
    setPhase('FEEDBACK');
  }, [config.nBackLevel, history, turnCount, activeElo]);

  const handleContinue = useCallback(() => {
    if (history.length < config.nBackLevel) setPhase('WARMUP');
    else setPhase('PLAYING');
    nextTurn();
  }, [history.length, config.nBackLevel, nextTurn]);
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-black text-slate-200 font-sans flex flex-col p-2 md:p-4 selection:bg-purple-500/40 relative">
      ...
      <div className="flex-1 w-full flex flex-col items-center justify-center relative z-10 overflow-hidden">
        ...
        {(phase === 'WARMUP' || phase === 'PLAYING') && currentItem && (
          <div className="w-full h-full flex flex-col items-center justify-center animate-in fade-in zoom-in-95 duration-300 gap-4 md:gap-8">
            <div className={`w-full flex-1 flex flex-col lg:flex-row gap-4 items-center justify-center min-h-0 ${currentItem.stimulus.dictionaryPos === 'RIGHT' ? 'lg:flex-row-reverse' : ''}`}>
              <div className="flex-1 p-4 md:p-8 bg-slate-900 border border-slate-700 rounded-[2rem] shadow-2xl flex flex-col items-center justify-center gap-4 w-full h-full max-h-[60vh]"><VisualRenderer stim={currentItem.stimulus} /><div className="bg-black px-4 py-2 rounded-lg text-slate-300 font-mono text-sm md:text-lg border border-slate-800 tracking-wider shadow-inner mt-4">{currentItem.stimulus.textQuery}</div></div>
            ...
            <div className="w-full flex flex-col gap-4 flex-shrink-0">
              {config.baseTimer !== -1 ? (<div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden shadow-inner"><div className={`h-full transition-all duration-100 ease-linear ${timer < 3 ? 'bg-red-500' : 'bg-white'}`} style={{ width: `${(timer / config.baseTimer) * 100}%` }} /></div>) : (<div className="w-full h-2 bg-slate-800 rounded-full overflow-hidden shadow-inner flex items-center justify-center"><div className="w-full h-full bg-slate-800 animate-pulse" /></div>)}
        ...
        {phase === 'FEEDBACK' && logs.length > 0 && logs[0].nBackItem && (
          <div className="w-full h-full max-h-full flex flex-col animate-in zoom-in-95 overflow-hidden">
            <div className="bg-slate-900 border border-slate-700 rounded-3xl shadow-2xl flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-800 min-h-0">
                ...
                {/* LEFT SIDE: N-BACK ITEM */}
                <div className="p-4 flex flex-col items-center opacity-60 bg-slate-950/30">
                  <div className="text-[10px] font-bold text-slate-500 uppercase mb-2 flex items-center gap-2 px-3 py-1 bg-slate-900 rounded-full border border-slate-800">
                    <History className="w-3 h-3" /> N-{config.nBackLevel}
                  </div>
                  <div className="mb-2 scale-[0.65] origin-center">
                    <VisualRenderer stim={logs[0].nBackItem.stimulus} />
                  </div>

                  {/* NEW BLURRED BOX */}
                  <BlurredLogicBox
                    label="Target Logic"
                    result={logs[0].nBackItem.result}
                    proof={logs[0].nBackItem.stimulus.logicProof}
                  />
                </div>

                {/* RIGHT SIDE: CURRENT ITEM */}
                <div className="p-4 flex flex-col items-center bg-slate-900/50">
                  <div className="text-[10px] font-bold text-emerald-500 uppercase mb-2 flex items-center gap-2 px-3 py-1 bg-emerald-900/10 rounded-full border border-emerald-500/20">
                    <Activity className="w-3 h-3" /> Current
                  </div>
                  <div className="mb-2 scale-[0.75] origin-center">
                    <VisualRenderer stim={logs[0].currentItem.stimulus} />
                  </div>

                  {/* NEW BLURRED BOX */}
                  <BlurredLogicBox
                    label="Derived Logic"
                    result={logs[0].currentItem.result}
                    proof={logs[0].currentItem.stimulus.logicProof}
                    isCurrent={true}
                  />
                </div>
              </div>
              <div className="p-4 bg-slate-950 border-t border-slate-800 text-center flex flex-col items-center gap-3 flex-shrink-0">
                <div className="flex flex-col items-center gap-1"><div className="text-xs text-slate-400">Logic Check: <span className="text-white font-bold px-1">{logs[0].currentItem.result}</span> vs <span className="text-slate-400 font-bold px-1">{logs[0].nBackItem.result}</span></div><div className={`text-[10px] font-bold uppercase tracking-[0.2em] px-3 py-0.5 rounded-full ${logs[0].isMatch ? 'bg-blue-900/20 text-blue-400' : 'bg-slate-800 text-slate-500'}`}>{logs[0].isMatch ? 'MATCH FOUND' : 'NO MATCH'}</div></div>
